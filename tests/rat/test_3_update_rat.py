...
# Update a rat error 422
def test_update_rat_error422(request):
    # Configure test info in the report
    request.node._method = "PUT"
    request.node._route = "/rat"
    request.node._title = "Test update a rat error 422"
    request.node._description = "Pass if the status code is 422"

    with open("tests/test_data.yaml", "r") as yaml_file:
        data = yaml.safe_load(yaml_file)
        rat_data = data["update_rat_error422"][0]
        ...
        #Verify the result
        request.node._expected_status_code = 422
...
def test_update_rat_error400(request):
    ...
    with open("tests/test_data.yaml", "r") as yaml_file:
        data = yaml.safe_load(yaml_file)
        rat_data = data["update_rat_error400"][0]
